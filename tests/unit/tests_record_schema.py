...
class TestConsumerRecordSchema(unittest.TestCase):
    ...
    def test_all_byte_formats_validate_for_content_errors_and_headers(self) -> None:
        for bytes_format in BytesFormat:
            ...
            record = Record(
                ...
                value=b"Hello world",
                headers=[
                    Header("source", b"storefront", StringDeserializer(), bytes_format),
                    Header("binary", b"\xff", StringDeserializer(), bytes_format),
                    Header("nullable", None, StringDeserializer(), bytes_format),
                ],
                key_deserialization=Deserialization.JSON,
                value_deserialization=Deserialization.BYTES,
                key_deserializer=key_deserializer,
                key_bytes_format=bytes_format,
                value_bytes_format=bytes_format,
            )
    ...
    def test_schema_rejects_superseded_and_inconsistent_contracts(self) -> None:
        valid = Record(
            topic="orders",
            partition=0,
            offset=42,
            key=b"Hello world",
            key_bytes_format=BytesFormat.BASE64,
        ).dict()
        ...
        mismatched_format = deepcopy(valid)
        mismatched_format["key"]["deserializer"]["format"] = "BYTE_ARRAY"
        with self.assertRaises(ValidationError):
            self.validator.validate(mismatched_format)
        ...
        error = old_fallback["key"]["deserializer"]["error"]
        error["fallback"] = error.pop("deserializer")["type"]
        with self.assertRaises(ValidationError):
